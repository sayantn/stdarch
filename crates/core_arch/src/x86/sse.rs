...
mod tests {
    ...
    #[target_feature(enable = "sse")]
    fn m128_as_bytes(a: __m128) -> [u8; 16] {
        unsafe { transmute(a) }
    }

    #[target_feature(enable = "sse")]
    fn m128_bits(a: __m128) -> u32x4 {
        unsafe { transmute(a) }
    }
    ...
    fn test_mm_min_ps() {
        ...
        let r1 = m128_as_bytes(_mm_min_ps(a, b));
        let r2 = m128_as_bytes(_mm_min_ps(b, a));
        let a = m128_as_bytes(a);
        let b = m128_as_bytes(b);
        assert_eq!(r1, b);
        assert_eq!(r2, a);
    ...
    fn test_mm_max_ps() {
        ...
        let a = _mm_setr_ps(-0.0, 0.0, 0.0, 0.0);
        let b = _mm_setr_ps(0.0, 0.0, 0.0, 0.0);
        let r1 = m128_as_bytes(_mm_max_ps(a, b));
        let r2 = m128_as_bytes(_mm_max_ps(b, a));
        let a = m128_as_bytes(a);
        let b = m128_as_bytes(b);
        assert_eq!(r1, b);
        assert_eq!(r2, a);
    ...
    #[simd_test(enable = "sse")]
    fn test_mm_and_ps() {
        let a = _mm_set1_ps(f32::from_bits(0b0011));
        let b = _mm_set1_ps(f32::from_bits(0b0101));
        let r = _mm_and_ps(*black_box(&a), *black_box(&b));
        let e = _mm_set1_ps(f32::from_bits(0b0001));
        assert_eq_m128(r, e);
    }

    #[simd_test(enable = "sse")]
    fn test_mm_andnot_ps() {
        let a = _mm_set1_ps(f32::from_bits(0b0011));
        let b = _mm_set1_ps(f32::from_bits(0b0101));
        let r = _mm_andnot_ps(*black_box(&a), *black_box(&b));
        let e = _mm_set1_ps(f32::from_bits(0b0100));
        assert_eq_m128(r, e);
    }
    ...
    fn test_mm_or_ps() {
        ...
        let e = _mm_set1_ps(f32::from_bits(0b0111));
        assert_eq_m128(r, e);
    }
    ...
    fn test_mm_xor_ps() {
        ...
        let e = _mm_set1_ps(f32::from_bits(0b0110));
        assert_eq_m128(r, e);
    }
    ...
    fn test_mm_cmpeq_ss() {
        ...
        let r = m128_bits(_mm_cmpeq_ss(a, b));
        let e = m128_bits(_mm_setr_ps(f32::from_bits(0), 2.0, 3.0, 4.0));
        assert_eq!(r, e);

        let b2 = _mm_setr_ps(1.0, 5.0, 6.0, 7.0);
        let r2 = m128_bits(_mm_cmpeq_ss(a, b2));
        let e2 = m128_bits(_mm_setr_ps(f32::from_bits(0xffffffff), 2.0, 3.0, 4.0));
        assert_eq!(r2, e2);
    }
    ...
    fn test_mm_cmplt_ss() {
        ...
        let rb = m128_bits(_mm_cmplt_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmplt_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmplt_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmple_ss() {
        ...
        let rb = m128_bits(_mm_cmple_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmple_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmple_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpgt_ss() {
        ...
        let rb = m128_bits(_mm_cmpgt_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpgt_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpgt_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpge_ss() {
        ...
        let rb = m128_bits(_mm_cmpge_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpge_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpge_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpneq_ss() {
        ...
        let rb = m128_bits(_mm_cmpneq_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpneq_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpneq_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpnlt_ss() {
        ...
        let rb = m128_bits(_mm_cmpnlt_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpnlt_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpnlt_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpnle_ss() {
        ...
        let rb = m128_bits(_mm_cmpnle_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpnle_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpnle_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpngt_ss() {
        ...
        let rb = m128_bits(_mm_cmpngt_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpngt_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpngt_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpnge_ss() {
        ...
        let rb = m128_bits(_mm_cmpnge_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpnge_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpnge_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpord_ss() {
        ...
        let rb = m128_bits(_mm_cmpord_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpord_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpord_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpunord_ss() {
        ...
        let rb = m128_bits(_mm_cmpunord_ss(a, b));
        let eb = m128_bits(_mm_setr_ps(f32::from_bits(b1), 2.0, 3.0, 4.0));
        assert_eq!(rb, eb);

        let rc = m128_bits(_mm_cmpunord_ss(a, c));
        let ec = m128_bits(_mm_setr_ps(f32::from_bits(c1), 2.0, 3.0, 4.0));
        assert_eq!(rc, ec);

        let rd = m128_bits(_mm_cmpunord_ss(a, d));
        let ed = m128_bits(_mm_setr_ps(f32::from_bits(d1), 2.0, 3.0, 4.0));
        assert_eq!(rd, ed);
    }
    ...
    fn test_mm_cmpeq_ps() {
        ...
        let r = m128_bits(_mm_cmpeq_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmplt_ps() {
        ...
        let r = m128_bits(_mm_cmplt_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmple_ps() {
        ...
        let r = m128_bits(_mm_cmple_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpgt_ps() {
        ...
        let r = m128_bits(_mm_cmpgt_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpge_ps() {
        ...
        let r = m128_bits(_mm_cmpge_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpneq_ps() {
        ...
        let r = m128_bits(_mm_cmpneq_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpnlt_ps() {
        ...
        let r = m128_bits(_mm_cmpnlt_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpnle_ps() {
        ...
        let r = m128_bits(_mm_cmpnle_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpngt_ps() {
        ...
        let r = m128_bits(_mm_cmpngt_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpnge_ps() {
        ...
        let r = m128_bits(_mm_cmpnge_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpord_ps() {
        ...
        let e = u32x4::new(tru, fls, fls, fls);
        let r = m128_bits(_mm_cmpord_ps(a, b));
        assert_eq!(r, e);
    }
    ...
    fn test_mm_cmpunord_ps() {
        ...
        let e = u32x4::new(fls, tru, tru, tru);
        let r = m128_bits(_mm_cmpunord_ps(a, b));
        assert_eq!(r, e);
    }
